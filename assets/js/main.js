  ...
  var dtSettings = {
    ...
    fnDrawCallback: function (oSettings) {
      _table_jump_to_page(this, oSettings);
        ...
        var paginationInfo = $(this).closest('.dataTables_wrapper').find('.dataTables_info');
        if(this.api().page.info().pages > 1){
            pagination.toggle(true);
        }
        if(isNaN(this.api().page.info().recordsTotal)) {
            pagination.toggle(false);
            paginationInfo.text('No data available in table');
        }
