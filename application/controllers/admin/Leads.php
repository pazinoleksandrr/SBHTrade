...
class Leads extends AdminController
{
        ...
        if(is_admin()){
            ...
            if(isset($staff->role_name)){
                if($staff->role_name == 'Lead'){
                    ...
                    $data['summary'] = get_leads_summary('', implode(',', $staff_ids));
                }
                if($staff->role_name == 'Sales'){
                    $data['summary'] = get_leads_summary($staff_id);
                }
            }
        }*/
        $data['staff'] = [];
    ...
    public function kanban()
    {
        ...
        $data['statuses']      = $this->leads_model->get_status('', ['name !=' => 'customer', 'id !=' => 1, 'color !=' => '#7cb342', 'isdefault !=' => 1]);
    ...
    public function lead($id = '')
    {
        ...
        if (!is_staff_member() || ($id != '' && !$this->leads_model->staff_can_access_lead($id, '', $staff_ids_of_dep))) {
            ajax_access_denied();
        }
        ...
        if ($this->input->post()) {
            if ($id == '') {
                ...
                if ($success) {
                    ...
                    $message = _l('updated_successfully', _l('client_'));
                }
                echo json_encode([
                    'success'          => $success,
                    'message'          => $message,
    ...
    private function _get_lead_data($id = '')
    {
        ...
        if(is_admin()){
            ...
            if(isset($staff->role_name)){
                if($staff->role_name == 'Lead'){
                    $staff_ids = $this->staff_model->get_staff_ids_by_lead_id($staff_id);//print_r($staff_ids);die();
                    if(!empty($staff_ids)) $staff_ids = array_map('implode', $staff_ids);//print_r([$staff_ids]);die();
                    $data['members'] = $this->staff_model->get('', 'staffid in ('.implode(',', $staff_ids).') and active = 1 and is_not_staff = 0');
                    $staff_ids_of_dep = implode(',', $staff_ids);
                }
                if($staff->role_name == 'Sales'){
                    $data['members']       = $this->staff_model->get('', ['is_not_staff' => 0, 'active' => 1, 'staffid' => $staff_id]);
                }
            }
        }
        ...
        if (is_numeric($id)) {
            if($staff_ids_of_dep == '') $leadWhere = (has_permission('leads', '', 'view') ? [] : '(assigned = ' . get_staff_user_id() . ' OR addedfrom=' . get_staff_user_id() . ' OR is_public=1)');
            else $leadWhere = (has_permission('leads', '', 'view') ? [] : '(assigned in (' . $staff_ids_of_dep . ') OR addedfrom=' . get_staff_user_id() . ' OR is_public=1)');
    ...
    function clean_leads(){
        if(is_admin()){
            $result = $this->leads_model->clean_leads();
            set_alert('success', _l('deleted', _l('client_')).' : '.$result);
	...
	$this->load->model(['staff_model']);
        ...
        if(!is_admin()){
            ...
            if(isset($staff->role_name)){
                if($staff->role_name == 'Lead'){
                    $staff_ids = $this->staff_model->get_staff_ids_by_lead_id($staff_id);
                    if(!empty($staff_ids)) $staff_ids = array_map('implode', $staff_ids);
                    $where_clients = ' assigned in ('.implode(',', $staff_ids).')';
                }
                if($staff->role_name == 'Sales'){
                    $where_clients = ' (assigned ='.$staff_id.')';
                }
            }
	}
	

        //$where_clients_without_sync_record = $where_clients;
        $where_clients .= $where_clients == '' ? '' : ' AND';
        //$where_clients .= ' '.db_prefix().'leads.id NOT IN (select rel_id from '.db_prefix().'sync where rel_type = "client" AND status != 2) AND '.db_prefix().'leads.user_id = 0';
        //$where_clients .= ' NOT EXISTS (select * from '.db_prefix().'sync where rel_type = "client" AND '.db_prefix().'leads.id = '.db_prefix().'sync.rel_id)';
        $where_clients .= ' '.db_prefix().'leads.user_id = 0';
        if (!$id || !is_numeric($id)) {
            //check for post data
            //redirect(admin_url('leads'));
            $total_sync = 0;
            if ($this->input->post() && $this->input->post('mass_sync')) {
                $errors = [];
                $ids                   = $this->input->post('ids');
                $has_permission_sync = true;//has_permission('leads', '', 'sync');
                if (is_array($ids)) {
                    foreach ($ids as $id_) {
                        if ($this->input->post('mass_sync')) {
                            //if($this->leads_model->get($id_, $where_clients_without_sync_record.' '.db_prefix().'leads.user_id = 0')) $total_sync++;
                            if ($has_permission_sync && $client = $this->leads_model->get($id_, $where_clients)) {
                                /*$clients_to_sync[] = [
                                    'rel_id' => $client->id,
                                    'rel_type' => 'client',
                                    'status' => '1'
                                ];*/
                                //$response = $this->leads_model->sync($clients_to_sync);
                                $response = sync_client($client);
                                if ($response['status'] == 'success') {
                                    $total_sync++;
                                }else $errors[] = _l($response['message']).': '.$client->email;
                            }
                        }
                    }
                }
                $message = _l('total_clients_synced', $total_sync);
                if(!empty($errors)) set_alert('warning', $message.'<br>'.implode('<br>', $errors));
                else set_alert('success', $message);
            }
	}else{

            $client = $this->leads_model->get($id, $where_clients);
            if($client){
                /*$clients_to_sync[] = [
                    'rel_id' => $client->id,
                    'rel_type' => 'client',
		...
		$response = sync_client($client);		   
                $message = ($response['status'] == 'success') ? _l('client_synced') : _l($response['message']);
                set_alert($response['status'], $message);
            }
            //$response = $this->leads_model->sync($clients_to_sync);

            /*if ($response > 1) {
                set_alert('success', _l('total_leads_sent_to_sync', $response));
            } elseif ($response < 1 && $response > 0) {
                set_alert('success', _l('lead_sent_to_sync'));
            } else {
                if($this->leads_model->get($id, $where_clients_without_sync_record)) set_alert('success', _l('lead_sent_to_sync'));
                else set_alert('warning', _l('failed_to_sent_sync'));
            }*/
    ...
    public function delete($id)
    {
        ...
        $response = $this->leads_model->delete_soft($id);
        if (is_array($response) && isset($response['referenced'])) {
            set_alert('warning', _l('is_referenced', _l('lead_lowercase')));
        } elseif ($response === true) {
            set_alert('success', _l('deleted', _l('client_')));
        } else {
        ...
        if ($this->input->post()) {
            ...
            if ($id) {
                ...
                if (is_gdpr() && get_option('gdpr_after_lead_converted_delete') == '1') {
                    ...
                    $this->leads_model->delete_soft($data['leadid']);
                ...
                foreach ($ids as $id) {
                    if ($this->input->post('mass_delete')) {
                        if ($has_permission_delete) {
                            if ($this->leads_model->delete_soft($id)) {
                                $total_deleted++;
                            }
                        }
                    } else {
