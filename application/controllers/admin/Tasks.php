...
class Tasks extends AdminController
{
    ...
    public function list_tasks($id = '')
    {
        ...
        $data['types']  = $this->tasks_model->get_types();
    ...
    {
        blank_page(_l('page_not_found_'));die();
        $overview = [];
        ...
        for ($m = 1; $m <= 12; $m++) {
            ...
            $this->db->where('CASE rel_type WHEN "lead" THEN rel_id IN (SELECT id FROM '.db_prefix().'leads WHERE id=rel_id AND '.db_prefix().'leads.deleted = 0 AND '.db_prefix().'leads.blocked = 0) ELSE 1=1 END');
        ...
        if(is_admin()){
            ...
            if(isset($staff->role_name)){
                if($staff->role_name == 'Lead'){
                    $staff_ids = $this->staff_model->get_staff_ids_by_lead_id($staff_id);//print_r($staff_ids);die();
                    if(!empty($staff_ids)) $staff_ids = array_map('implode', $staff_ids);//print_r([$staff_ids]);die();
                    $data['members'] = $this->staff_model->get('', 'staffid in ('.implode(',', $staff_ids).')');
                }
                if($staff->role_name == 'Sales'){
                    $data['members'] = $this->staff_model->get('', ['staffid' => $staff_id]);
                }
            }
        }
        $data['id']      = $id;
    ...
    public function get_clients_based_staff_role()
    {
        if ($this->input->post()) {
            ...
            if(is_admin()){
                ...
                if(isset($staff->role_name)){
                    if($staff->role_name == 'Lead'){
                        ...
                        if(!empty($staff_ids)) $staff_ids = array_map('implode', $staff_ids);//print_r([$staff_ids]);die();
                        $staff_ids[] = 0;
                        $data = get_relation_data($type, '', ['seller_ids' => $staff_ids]);//print_r($data);die();
                        $relOptions = init_relation_options($data, $type, $rel_id, $staff_ids);
                    }
                    if($staff->role_name == 'Sales'){
                        $data = get_relation_data($type, '', ['seller_id' => $staff_id]);
                        $relOptions = init_relation_options($data, $type, $rel_id);
                    }
                }
            }
