...
class TasksKanban extends AbstractKanban
{
    ...
    protected function initiateQuery()
    {
        ...
        $this->ci->db->where('CASE rel_type WHEN "lead" THEN rel_id IN (SELECT id FROM '.db_prefix().'leads WHERE id=rel_id AND '.db_prefix().'leads.deleted = 0 AND '.db_prefix().'leads.blocked = 0) ELSE 1=1 END');
