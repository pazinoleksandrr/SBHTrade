    ...
    public function get_calendar_data($start, $end, $client_id = '', $contact_id = '', $filters = false)
    {
        ...
        if (get_option('show_tasks_on_calendar') == 1 && !$ff || $ff && array_key_exists('tasks', $filters)) {
            if ($client_data && !$has_contact_permission_projects) {
                ...
                $this->db->select(db_prefix() . 'tasks.name as title,' . db_prefix() . 'tasks.id as id,' . tasks_rel_name_select_query() . ' as rel_name,rel_id,' . db_prefix() . 'tasks.status as status,milestone,CASE WHEN duedate IS NULL THEN startdate ELSE duedate END as date', false);
                ...
                $this->db->join(db_prefix() . 'leads', db_prefix() . 'leads.id=' . db_prefix() . 'tasks.rel_id AND ' . db_prefix() . 'tasks.rel_type = "lead"', 'left');
                $this->db->where('(CASE WHEN rel_type="lead" THEN ' . db_prefix() . 'leads.deleted = 0 AND ' . db_prefix() . 'leads.blocked = 0 ELSE 1=1 END)', null, false);
