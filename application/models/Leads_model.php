...
class Leads_model extends App_Model
{
    ...
    {
        parent::__construct();
        $this->table_name = db_prefix() . 'leads';
    }
    ...
    public function get($id = '', $where = [])
    {
        $this->db->select('*,'
            ...
            . '(CASE ' . db_prefix() . 'leads.name WHEN "" THEN 
                        (CASE ' . db_prefix() . 'leads.email WHEN "" THEN concat(first_name, " ", last_name) ELSE ' . db_prefix() . 'leads.email END) 
                    ELSE ' . db_prefix() . 'leads.name END) as name, '

            . db_prefix() . 'leads.id,' . db_prefix() . 'leads_status.name as status_name,' . db_prefix() . 'leads_sources.name as source_name');
    ...
    public function add($data)
    {
        ...
        if (isset($data['name']) && $data['name'] != '') {
            ...
            if (count($pieces) > 1) {
                $data['last_name']  = array_pop($pieces);
                $data['first_name'] = implode(' ', $pieces);
            }
            else {
    ...
    public function delete_soft($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            $this->log_lead_activity($id, 'not_lead_activity_deleted');

            log_activity('Lead Deleted [ID: ' . $id . ']');
...
//                    'rel_id' => $client->id,
//                    'rel_type' => 'client',
//                    'status' => 1 //1-need_sync,2-synced,3-failed
//                ];*/
//                $this->db->insert(db_prefix() . 'sync', $client);//$sync_data);
//                $insert_id = $this->db->insert_id();
//                if ($insert_id) {
//                    log_activity('New Sync Request Added for Client [ID: ' . $insert_id . ']');
//                    $this->log_lead_activity($insert_id, 'client_sync_requested');
//                }
//            }
//        }
//        return $affectedRows;
...
//    public function update_sync_request($data, $where)
//    {
//        $affectedRows = 0;
//        $this->db->where($where);
//        $this->db->update(db_prefix() . 'sync', $data);
//        if ($this->db->affected_rows() > 0) {
//            $affectedRows++;
//            log_activity('Client Sync Request Completed [ID: ' . $where['rel_id'] . ']');
//            return true;
//        }
//        if ($affectedRows > 0) {
//            return true;
...
//    public function get_sync($where = [])
//    {
...
//        $this->db->select(db_prefix().'leads.id as crm_user_id, '.db_prefix().'leads.user_id, '.db_prefix().'leads.email, '.db_prefix().'leads.first_name, '.db_prefix().'leads.last_name,
//                            '.db_prefix().'leads.default_language as language,
//                            '.db_prefix().'countries.short_name as country, '.db_prefix().'leads.phonenumber as phone, (case when '.db_prefix().'leads.blocked = 0 then "false" else "true" end) as blocked,
//                            (case when '.db_prefix().'leads.deleted = 0 then "false" else "true" end) as is_deleted, '.db_prefix().'leads.city, '.db_prefix().'leads.address as street,
//                            '.db_prefix().'leads.zip as post_code, '.db_prefix().'leads.dateadded as created_at');
//        //$this->db->join(db_prefix() . 'leads_status', db_prefix() . 'leads_status.id=' . db_prefix() . 'leads.status', 'left');
//        //$this->db->join(db_prefix() . 'leads_sources', db_prefix() . 'leads_sources.id=' . db_prefix() . 'leads.source', 'left');
//        $this->db->join(db_prefix() . 'countries', db_prefix() . 'leads.country = ' . db_prefix() . 'countries.country_id', 'left');
//        $this->db->join(db_prefix() . 'sync', db_prefix() . 'sync.rel_id=' . db_prefix() . 'leads.id AND '.db_prefix().'sync.rel_type = "client"', 'left');
//
//        $this->db->where('deleted = 0 AND blocked = 0');
//        if(!empty($where)) $this->db->where($where);
//        else $this->db->where(db_prefix().'sync.status != 2');
//
//        return $this->db->get(db_prefix() . 'leads')->result_array();
//    }
    ...
    public function staff_can_access_lead($id, $staff_id = '', $dep_staffs = '')
    {
        ...
        if ($dep_staffs == '') {
            ...
            if (total_rows(db_prefix() . 'leads', 'id="' . $CI->db->escape_str($id) . '" AND (assigned in (' . $CI->db->escape_str($dep_staffs) . ') OR is_public=1 OR addedfrom=' . $CI->db->escape_str($staff_id) . ')') > 0) {
                return true;
            }
        }
    ...
    public function log_lead_activity($id, $description, $integration = false, $additional_data = '')
    {
        $log = [
            ...
            'leadid'          => $id,
            'staffid'         => get_staff_user_id() ? get_staff_user_id() : "Added from Api",
            'additional_data' => $additional_data,
            'full_name'       => get_staff_user_id() ? get_staff_full_name(get_staff_user_id()) : "Added from Api",
        ];
