...
class Staff_model extends App_Model
{
    ...
    public function get_with_role($id = '', $where = [])
    {
        $select_str = '*,CONCAT(firstname,\' \',lastname) as full_name, '.db_prefix().'roles.name as role_name';
    ...
    public function get_staff_ids_by_lead_id($lead_staff){
        try {
            $department = $this->db->select('*')->where(['name' => 'Department', 'slug' => 'staff_department', 'type' => 'select'])->get(db_prefix().'customfields')->row_array();
            if($department){
                $lead_dep = $this->db->select('*')->where(['relid' => $lead_staff, 'fieldid' => $department['id'], 'fieldto' => 'staff'])->get(db_prefix().'customfieldsvalues')->row_array();
                if($lead_dep){
                    $this->db->select(db_prefix().'staff.staffid');
                    $this->db->join(db_prefix().'staff', db_prefix().'staff.staffid = '.db_prefix().'customfieldsvalues.relid', 'left');
                    $this->db->where([
                        db_prefix().'customfieldsvalues.fieldid' => $lead_dep['fieldid'],
                        db_prefix().'customfieldsvalues.fieldto' => $lead_dep['fieldto'],
                        db_prefix().'customfieldsvalues.value' => $lead_dep['value'],
                    ]);
    ...
    public function get_with_dep($id = '', $where = [])
    {
        $select_str = '*,CONCAT(firstname,\' \',lastname) as full_name, cfv.value as department';
        ...
        if (is_numeric($id)) {
            $this->db->where('staffid', $id);
            $this->db->join('(select * from '.db_prefix().'customfieldsvalues where '.db_prefix().'customfieldsvalues.fieldid IN (SELECT '.db_prefix().'customfields.id from '.db_prefix().'customfields where '.db_prefix().'customfields.name = "Department" 
                            AND '.db_prefix().'customfields.slug = "staff_department" AND '.db_prefix().'customfields.type = "select" AND '.db_prefix().'customfields.active = 1)) cfv', 'cfv.relid = '.db_prefix().'staff.staffid AND cfv.fieldto = "staff"', 'left');
            /*$this->db->join(db_prefix().'customfieldsvalues', db_prefix().'customfieldsvalues.relid = '.db_prefix().'staff.staffid AND '.db_prefix().'customfieldsvalues.fieldto = "staff"', 'left');
            $this->db->join(db_prefix().'customfields', db_prefix().'customfields.id = '.db_prefix().'customfieldsvalues.fieldid AND '.db_prefix().'customfields.name = "Department"
                            AND '.db_prefix().'customfields.slug = "staff_department" AND '.db_prefix().'customfields.type = "select" AND '.db_prefix().'customfields.active = 1', 'left');*/
            $staff = $this->db->get(db_prefix() . 'staff')->row();
    ...
    public function get_with_voip_id($id = '', $where = [])
    {
        $select_str = '*,CONCAT(firstname,\' \',lastname) as full_name, cfv.value as voip_id';
        ...
        if (is_numeric($id)) {
            $this->db->where('staffid', $id);
            $this->db->join('(select * from '.db_prefix().'customfieldsvalues where '.db_prefix().'customfieldsvalues.fieldid IN (SELECT '.db_prefix().'customfields.id from '.db_prefix().'customfields where '.db_prefix().'customfields.name = "CommpeakLogin" 
                            AND '.db_prefix().'customfields.slug = "staff_commpeaklogin" AND '.db_prefix().'customfields.type = "input" AND '.db_prefix().'customfields.active = 1)) as cfv', 'cfv.relid = '.db_prefix().'staff.staffid AND cfv.fieldto = "staff"', 'left');
            $staff = $this->db->get(db_prefix() . 'staff')->row();
