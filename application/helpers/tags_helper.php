...
if(!function_exists('mask_phone')){
    function mask_phone($phone, $permission = null){
        if($permission == null) $permission = has_permission('phone_number', '', 'hide');
        if($permission && !is_admin()){
            $num = explode(' ', $phone);
            $area = count($num) > 1 ? strlen($num[0]) : 2;

            $num = substr($phone, 0, $area); // get the first two digits or area
            $num .= str_repeat( '*', ( strlen( $phone ) - (2 + $area) ) ); // apply enough asterisks to cover the middle numbers
            $num .= substr( $phone, -2 ); // get the last two digits
            return $num;
        }else return $phone;
    }
}