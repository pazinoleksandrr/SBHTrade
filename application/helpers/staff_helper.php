...
function get_available_staff_permissions($data = [])
{
    ...
    $addFinancePermission = true;
    if (isset($data['staff_id']) && $data['staff_id']) {
        $is_staff_member = is_staff_member($data['staff_id']);
        if (!$is_staff_member) {
            $addFinancePermission = false;
        }
    }

    if ($addFinancePermission) {
        $corePermissions['finance'] = [
            'name'         => _l('finance_'),
            'capabilities' => [
                'view'   => $viewGlobalName,
                'create_deposit' => _l('permission_create_deposit_'),
                'edit_deposit' => _l('permission_edit_deposit_'),
                'delete_deposit' => _l('permission_delete_deposit_'),
                'create_withdrawal' => _l('permission_create_withdrawal_'),
                'edit_withdrawal' => _l('permission_edit_withdrawal_'),
                'delete_withdrawal' => _l('permission_delete_withdrawal_'),
            ],
        ];
    }
...
function is_lead_staff($staff_id = '')
{
    $CI = & get_instance();
    if ($staff_id == '') {
        if (isset($GLOBALS['current_user']) && isset($GLOBALS['current_user']->role_name)) {
            return $GLOBALS['current_user']->role_name === 'Lead';
        }
        $staff_id = get_staff_user_id();
    }
    ...
    return isset($staff->role_name) && $staff->role_name == 'Lead';
}