...
function format_members_by_ids_and_names_full_name($ids, $names, $hidden_export_table = true, $image_class = 'staff-profile-image-small')
{
    $outputAssignees = '';
    $exportAssignees = '';

    $assignees   = is_array($names) ? explode(',', $names) : [$names];
    $assigneeIds = is_array($ids) ? explode(',', $ids) : [$ids];
    foreach ($assignees as $key => $assigned) {
        $assignee_id = $assigneeIds[$key];
        $assignee_id = trim($assignee_id);
        if ($assigned != '') {
            $outputAssignees .= $outputAssignees == '' ? '' : ' | ';
            $outputAssignees .= '<a href="' . admin_url('profile/' . $assignee_id) . '">' .$assigned. '</a>';
            $exportAssignees .= $assigned . ', ';
        }
    }
...
function tasks_summary_data($rel_id = null, $rel_type = null)
{
    ...
    foreach ($statuses as $status) {
        ...
        if ($rel_id && $rel_type) {
            if($rel_type = 'lead'){
                $tasks_where .= ' AND rel_id=' . $CI->db->escape_str($rel_id) . ' AND rel_type="' . $CI->db->escape_str($rel_type) . '" AND rel_id IN (SELECT id FROM '.db_prefix().'leads WHERE id=rel_id AND '.db_prefix().'leads.deleted = 0 AND '.db_prefix().'leads.blocked = 0)';
                $tasks_my_where .= ' AND rel_id=' . $CI->db->escape_str($rel_id) . ' AND rel_type="' . $CI->db->escape_str($rel_type) . '" AND rel_id IN (SELECT id FROM '.db_prefix().'leads WHERE id=rel_id AND '.db_prefix().'leads.deleted = 0 AND '.db_prefix().'leads.blocked = 0)';
            }else{
            ...
            $sqlProjectTasksWhere = ' AND (CASE rel_type
            WHEN "project"
            THEN rel_id NOT IN (SELECT project_id FROM ' . db_prefix() . 'project_settings WHERE project_id=rel_id AND name="hide_tasks_on_main_tasks_table" AND value=1)
            WHEN "lead" 
            THEN rel_id IN (SELECT id FROM '.db_prefix().'leads WHERE id=rel_id AND '.db_prefix().'leads.deleted = 0 AND '.db_prefix().'leads.blocked = 0)
            ELSE 1=1
            END)';
...
function get_tasks_where_string($table = true)
{
    ...
    if(!is_admin() && !has_permission('leads', '', 'view')){
        ...
        $staff = $CI->staff_model->get_with_role($staff_id);
        if(isset($staff->role_name)){
            if($staff->role_name == 'Lead'){
                $staff_ids = $CI->staff_model->get_staff_ids_by_lead_id($staff_id);
                if(!empty($staff_ids)) $staff_ids = array_map('implode', $staff_ids);
                $where_tasks = ' in ('.implode(',', $staff_ids).')';
            }
            if($staff->role_name == 'Sales'){
                $where_tasks = '='.$staff_id;
            }
        }
    }
    //$_tasks_where = '(' . db_prefix() . 'tasks.id IN (SELECT taskid FROM ' . db_prefix() . 'task_assigned WHERE staffid = ' . get_staff_user_id() . ') OR ' . db_prefix() . 'tasks.id IN (SELECT taskid FROM ' . db_prefix() . 'task_followers WHERE staffid = ' . get_staff_user_id() . ') OR ('.db_prefix() . 'tasks.addedfrom=' . get_staff_user_id() . ' AND '.db_prefix() . 'tasks.is_added_from_contact=0)';
    $_tasks_where = '(' . db_prefix() . 'tasks.id IN (SELECT taskid FROM ' . db_prefix() . 'task_assigned WHERE staffid ' . $where_tasks . ') OR ' . db_prefix() . 'tasks.id IN (SELECT taskid FROM ' . db_prefix() . 'task_followers WHERE staffid ' . $where_tasks . ') OR ('.db_prefix() . 'tasks.addedfrom' . $where_tasks . ' AND '.db_prefix() . 'tasks.is_added_from_contact=0)';
    ...
    $_tasks_where .= ' OR '.db_prefix() . 'tasks.is_public = 1)';
    if ($table == true) {
