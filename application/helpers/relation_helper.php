...
function get_relation_data($type, $rel_id = '', $extra = [])
{
    ...
    if ($type == 'customer' || $type == 'customers') {
        ...
        if ($rel_id != '') {
            ...
            $lead_staff = false;
            $where_clients = db_prefix() . 'leads.junk=0 AND '.db_prefix().'leads.deleted=0 AND '.db_prefix().'leads.blocked = 0';
            if (isset($extra['seller_id']) && $extra['seller_id'] != '') {
                $where_clients .= ' AND '. db_prefix() . 'leads.assigned='. $extra['seller_id'];
            }
            if (isset($extra['seller_ids']) && !empty($extra['seller_ids'])) {
                $ids = implode(',', $extra['seller_ids']);
//                $where_clients .= ' AND '. db_prefix() . 'leads.assigned in ('.$ids.')';
                $lead_staff = db_prefix().'leads.assigned in ('.$ids.')';
            }
            $search = $CI->misc_model->_search_leads($q, 0, $where_clients, $lead_staff);
            $data = $search['result'];
        }
 ...
 * @return string
 */
function init_relation_options($data, $type, $rel_id = '', $lead_staff = false)
{
    $_data = [];
    ...
    foreach ($data as $relation) {
        ...
        if ($type == 'project') {
            ...
            if (!has_permission('leads', '', 'view') && !$lead_staff) {
            ...
            if($lead_staff){
                if(!in_array($relation['assigned'], $lead_staff) &&  $relation['addedfrom'] != get_staff_user_id() && $relation['is_public'] != 1 && $rel_id != $relation_values['id']){
                    continue;
                }
            }
