...
$statuses              = $this->ci->leads_model->get_status('', ['name !=' => 'customer', 'id !=' => 1, 'color !=' => '#7cb342', 'isdefault !=' => 1]);

$aColumns = [
    '1',
    db_prefix() . 'leads.id as id',
    '(case when '.db_prefix() . 'leads.user_id = "" or '.db_prefix() . 'leads.user_id is null then "no" else "yes" end) as trader',
...
$aColumns = array_merge($aColumns, [/*'company',*/
    ...
    db_prefix() . 'countries.short_name as country',
    /*'lead_value',*/
...
$join = [
    ...
    'LEFT JOIN ' . db_prefix() . 'customfieldsvalues ON ' . db_prefix() . 'customfieldsvalues.relid = ' . db_prefix() . 'leads.assigned'.$join_statement,
...
$where  = [' AND deleted = 0 AND blocked = 0'];
$filter = false;
...
if ($this->ci->input->post('from_date')) {
    array_push($where, 'AND '.db_prefix().'leads.dateadded >= "'.$this->ci->db->escape_str($this->ci->input->post('from_date')).'"');
}

if ($this->ci->input->post('to_date')) {
    array_push($where, 'AND '.db_prefix().'leads.dateadded <= "'.$this->ci->db->escape_str($this->ci->input->post('to_date')).'"');
}

if ($this->ci->input->post('country')) {
    array_push($where, 'AND '.db_prefix().'leads.country =' . $this->ci->db->escape_str($this->ci->input->post('country')));
}
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, $additionalColumns, 'GROUP by '.db_prefix().'leads.id');

$output  = $result['output'];
$rResult = $result['rResult'];

$permission_phone = has_permission('phone_number', '', 'hide');
$sync_help_letter = '<i class="fa-regular fa-circle-question pull-left tw-mt-0.5 tw-mr-1" data-toggle="tooltip" data-title="'._l("click_to_sync_").'" data-original-title="" title=""></i>';
foreach ($rResult as $aRow) {
    ...
    if($aRow["trader"] == "yes"){
        $row[] = '<span class="text-success">'._l($aRow['trader'].'_').'</span>';//render_tags(_l($aRow['trader'].'_'));
    }else{
        $row[] = '<a href="'.admin_url('leads/sync/'.$aRow['id']).'" class="_sync text-danger">'._l($aRow['trader'].'_').'</a>'.$sync_help_letter;//render_tags(_l($aRow['trader'].'_'));
    }
    ...
    $row[] = ($aRow['phonenumber'] != '' ? '<a href="'.admin_url('voip/voip_call/'.$aRow['id']).'">' . mask_phone($aRow['phonenumber'], $permission_phone) . '</a>' : '');
    ...
    if ($aRow['assigned'] != 0) {
        ...
        $assignedOutput = format_members_by_ids_and_names_full_name($aRow['assigned'], $full_name);
