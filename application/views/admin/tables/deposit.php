...
$has_permission_edit = has_permission('finance', '', 'edit_deposit');
$has_permission_delete = has_permission('finance', '', 'delete_deposit');
...
$where  = [$starter.db_prefix() . 'finance.type = "deposit" AND deleted = 0 AND blocked = 0'];
$filter = false;
...
$aColumns = hooks()->apply_filters('deposit_table_sql_columns', $aColumns);
...
foreach ($rResult as $aRow) {
    ...
    $hrefAttr = 'href="' . admin_url('finance/deposit/' . $aRow['id']) . '" onclick="init_deposit(' . $aRow['id'] . ');return false;"';
    $nameRow  = '<a ' . $hrefAttr . '>' . $aRow['id'] . '</a>';
    $nameRow .= '<div class="row-options">';
    $nameRow .= '<a ' . $hrefAttr . '>' . _l('view') . '</a>';

    if ($has_permission_edit) {
        $nameRow .= ' | <a href="' . admin_url('finance/deposit/' . $aRow['id'] . '?edit=true') . '" onclick="init_deposit(' . $aRow['id'] . ', true);return false;">' . _l('edit') . '</a>';
    }

    if ($aRow['addedfrom'] == get_staff_user_id() || $has_permission_delete) {
        $nameRow .= ' | <a href="' . admin_url('finance/delete_deposit/' . $aRow['id']) . '" class="_delete text-danger">' . _l('delete') . '</a>';
    }
    $nameRow .= '</div>';
...
//    $row[] = _d($aRow['dateadded']);
    ...
    $row = hooks()->apply_filters('deposit_table_row_data', $row, $aRow);
